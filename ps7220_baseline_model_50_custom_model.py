...
# ################################################################################
prefix = 'ps7220'
workname = '50_line crack baseline model 모델 수정'
...
Net.classifier[4] = torch.nn.Conv2d(in_channels4, out_channels4, kernel_size=(1, 1), stride=(1, 1))
...
def cal_acc(segs, anns,batch_size):
    ...
    for i in range(batch_size):
        ...
        TP_px = torch.sum((torch.tensor(seg) == ann)*(ann == 1)).item()
        TN_px = torch.sum((torch.tensor(seg) == ann)*(ann == 0)).item()
        FP_px = torch.sum((torch.tensor(seg) != ann)*(ann == 0)).item()
        FN_px = torch.sum((torch.tensor(seg) != ann)*(ann == 1)).item()
...
# ################################################################################
# 모델 정확도 저장할 df 작성
model_df = pd.DataFrame(columns=['itr','loss_val','acc','precision','recall','TP','TN','FP','FN'])
